<template>
  <div class="wrapper" v-if="current_anime">
    <div class="background-wrap">
      <div class="background-top" :style="{backgroundImage: `url('${wallpapertest}')`}"></div>
    ...
    <article>
      <div style="padding: 20px; flex: 1;">
          ...
          <div style="margin: 15px 0px;">
            <div class="info-sec">
              ...
              <ul class="genres">
                <li
                  class="genre"
                  v-for="genre in current_anime.genres"
                  :key="genre.id"
                >{{genre.name}}</li>
              </ul>
            </div>
            ...
            <div class="info-sec">
              ...
              <ul class="genres" style="margin: 0px; align-self: center;">
                <li>
                  <button
                    @click="show_trailer = true"
                    class="trailer-link"
                  >{{current_anime.info.title}}</button>
                </li>
              </ul>
              <div class="trailer-wrapper" @click="show_trailer = false" v-if="show_trailer">
                <iframe
                  :src="`https://youtube.com/embed/${current_anime.info.youtube_trailer_id}`"
                  frameborder="0"
                ></iframe>
        ...
        <div class="next-up" v-if="expectingNext && !finishedAnime">
          ...
          <episode-card
            v-if="nextEpisode"
            :poster="animeWithDetails.poster"
            :episode="nextEpisode"
            @watchit="watchEpisode(nextEpisode)"
            style="margin: 0px;"
          ></episode-card>
        ...
        <div class="next-up" v-if="wasWatching && !expectingNext">
          ...
          <episode-card
            :poster="animeWithDetails.poster"
            :episode="wasWatching"
            @watchit="watchEpisode(wasWatching)"
            style="margin: 0px;"
          ></episode-card>
        </div>
      </div>
        ...
        <div>
          ...
          <ul class="episodes">
            <li v-for="(episode, index) in current_anime.episodes">
              <episode-card
                :poster="animeWithDetails.poster"
                :showLoader="index === start_episode"
                :episode="episode"
                @watchit="watchEpisode(episode)"
              ></episode-card>
            </li>
          </ul>
        </div>
...
export default {
  ...
  components: {
    ...
    headerr: () => import("./components/header.vue"),
    HeartIcon
  },
  beforeMount() {
    console.log("anime profile mounted", this.$route.params);
    this.getAnimeDetails(this.$route.params.id).then(() => {
      ...
      if (this.region_blocked) {
        ...
        const selectedWallpaper = `https://cdn.masterani.me/wallpaper/0/${
          wallpapers[this.getRandomInt(wallpapers.length)].file
        }`;
        this.loadWallpaper(selectedWallpaper);
      } else {
        if (!wallpapers || wallpapers.length === 0)
          this.wallpapertest = this.animeWithDetails.poster;
        this.wallpapertest = `https://cdn.masterani.me/wallpaper/0/${
          wallpapers[this.getRandomInt(wallpapers.length)].file
        }`;
      }
    }); //fetch the anime details again. this includes episodes.
  },
  computed: {
    ...mapState({
    ...
    isFavorite() {
      return this.favorite_animes.find(
        anime => anime.info.id === this.current_anime.info.id
      );
    },
    animeWithDetails() {
      return (
        this.animes_w_details &&
        this.animes_w_details[this.current_anime.info.id]
      );
    },
    wasWatching() {
    ...
    finishedAnime() {
      ...
      return (
        !episodes[watching] &&
        info.status === 0 &&
        this.wasWatching.current_time === 0
      );
    }
  },
  methods: {
    ...
    watchEpisode(_episode) {
      ...
      this.getVideoLinks({ slug, episode })
        .then(result => {
          ...
          this.$router.push(
            `/anime/${this.current_anime.info.id}/${
              this.current_anime.info.slug
            }/watch/${episode}`
          );
        })
        .catch(err => {
    ...
    loadWallpaper(selectedWallpaper) {
...
.wrapper {
  ...
  .next-up {
    border-top: solid 1px rgba(white, 0.09);
    margin-top: 30px;
  }

  .episodes-section {
    border-left: solid 1px rgba(white, 0.09);
    padding: 20px;
    display: flex;
    flex-direction: column;
